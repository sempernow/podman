...
## If user is local and has login shell, then Podman does this automatically upon adduser.
## Script generated by Deepseek : deepseek_bash_20250502_41a08f
...
assign_subids() {
    ...
    if grep -q "^$USERNAME:" "$file"; then
        echo "✅ $USERNAME already has $type ranges in $file"
        return
    fi

    # Add new entry
    echo "$USERNAME:$next_start:$RANGE_SIZE" | sudo tee -a "$file" >/dev/null
    echo "➕ Added $type range $next_start-$((next_start + RANGE_SIZE - 1)) to $file"
}
...
# Verify
echo -e "\n🔍 Current ranges for $USERNAME:"
grep "^$USERNAME:" /etc/subuid /etc/subgid || {
    echo "❌ Failed to assign ranges!"
    exit 1
}
...
sudo -u "$USERNAME" podman system migrate 2>/dev/null && \
    ...
    echo "⚠️ Podman migration skipped (not configured yet)"